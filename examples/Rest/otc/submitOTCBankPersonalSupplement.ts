...
async function submitOTCBankPersonalSupplementExample() {
  try {
    ...
    // Field names must match GET /otc/bank/bank_supplement_checklist codes
    const readDoc = (envKey: string, fallback: string) =>
      readFileSync(process.env[envKey] || fallback);

    const result = await gateRestClient.submitOTCBankPersonalSupplement({
      bank_id,
      id_document_front: {
        filename: 'id_front.jpg',
        content: readDoc('OTC_ID_FRONT_FILE', './id_front.jpg'),
      },
      id_document_back: {
        filename: 'id_back.jpg',
        content: readDoc('OTC_ID_BACK_FILE', './id_back.jpg'),
      },
      address_proof: {
        filename: 'address_proof.pdf',
        content: readDoc('OTC_ADDRESS_PROOF_FILE', './address_proof.pdf'),
      },
    });
